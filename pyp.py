...
class PypConfig:
    def __init__(self) -> None:
        ...
        self.unqualified_imports: List[str] = []

        self.shebang: str = "#!/usr/bin/env python3"
        if config_contents.startswith("#!"):
            next_line = config_contents.find("\n")
            self.shebang = config_contents[: next_line if next_line >= 0 else None]
        ...
        def add_requires(index: int, undefs: Set[str]) -> None:
            self.requires[index].update(undefs - BUILTINS)
        ...
        def inner(index: int, part: ast.AST) -> None:
            if isinstance(part, (ast.FunctionDef, ast.AsyncFunctionDef, ast.ClassDef)):
                ...
                if "*" in defs:
                    ...
                    self.unqualified_imports.append(part.module)
                add_defs(index, defs)
                ...
                # This allows us to do some amount of conditional definition
...
class PypTransform:
    ...
    def build_missing_imports(self) -> None:
        ...
        unqualified_imports = ["itertools", "math", "collections"] + self.config.unqualified_imports
        subimports = {
            name: module for module in unqualified_imports for name in get_names_in_module(module)
        }
